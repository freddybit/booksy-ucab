...
const filteredBooks = computed(() => {
  const targetCategory = (props.category || '').toLowerCase();
  if (!targetCategory) return books.value || [];
  return (books.value || []).filter(b => {
    const bookCategories = (b._categoryList || '').toString().toLowerCase();
    return bookCategories.includes(targetCategory);
  });
});
</script>
...
<template>
  <article class="catalog-article">
    <section class="section-left">
      <h1 class="catalog-title">CATÁLOGO: {{ category }}</h1> <p>{{ filteredBooks.length }} resultados</p>
    </section>
    <section id="catalog" class="section-right">
      ...
      <section v-else>
        ...
        <p v-if="books.length === 0">No hay libros cargados en el sistema</p>
      </section>
    </section>
  </article>
